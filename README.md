...
+ [一、安装说明（INSTALLATION）](#installation)
+ [二、连接数据库（CONNECTION）](#connection)
    + [2.1 连接数据库](#21)
    + [2.2 切换数据库连接](#22)
    + [2.3 使用同连接的其他数据库](#23)
+ [三、增删改查（CURD）](#curd)
    ...
    + [3.4 查](#34)
        ...
        + [3.4.2 字典条件查询](#342)
        + [3.4.3 IN 查询](#343-in-)
        + [3.4.4 LIKE 模糊查询](#344-like-)
        + [3.4.5 BETWEEN 查询](#345-between-)
        + [3.4.6 追加查询 and_where](#346-and_where)
        + [3.4.7 OR 查询 or_where](#347-or-or_where)
        + [3.4.8 其他操作符](#348)
+ [四、查询构建器（QUERY BUILDER）](#query-builder)
    + [4.1 选择字段 select](#41-select)
    + [4.2 联表查询 join](#42-join)
    + [4.3 分组及排序 group_by order_by](#43-group_by-order_by)
    + [4.4 结果筛选 having](#44-having)
    + [4.5 分页查询 skip limit](#45-skip-limit)
+ [五、执行原生SQL（RAW SQL）](#sqlraw-sql)
    + [5.1 执行原生SQL示例](#51-sql)
    + [5.2 使用助手函数来拼接SQL（防注入）](#52-sql)
+ [六、返回值（RETURNED VALUE）](#returned-value)
    + [6.1 统计 count](#61-count)
    + [6.2 多行 all](#62-all)
    + [6.3 单行 one](#63-one)
    + [6.4 单个值 scalar](#64-scalar)
    + [6.5 一列 column](#65-column)
    + [6.6 结果索引（一维）](#66)
    + [6.7 结果索引（二维）](#67)
    + [6.8 获取SQL](#68-sql)
    + [6.9 获取插入的ID](#69-id)
    + [6.10 获取影响的行数](#610)
+ [七、事务支持（TRANSACTION）](#transaction)
    + [7.1 上下文管理器](#71)
    + [7.2 装饰器](#72)
+ [八、单元测试（UNITTEST）](#unittest)
+ [九、防注入（INJECTION）](#injection)
+ [十、内置异常（EXCEPTIONS）](#exceptions)
+ [十一、请我喝奶茶（DONATION）](#donation)
